...
pub fn calculate_pressure_drop(
    ...
    // Assume average temperature for property evaluation
    let temperature = 650.0; // K (typical average)
...
/// # Returns
/// Pressure drop in Pascals
pub fn calculate_elevation_pressure_drop(height_change: f64, temperature: f64) -> f64 {
    const GRAVITY: f64 = 9.81; // m/s² (Earth gravity, adjust for space)
    let density = fluid_dynamics::sodium_density(temperature);
...
mod tests {
    ...
    fn test_pressure_drop() {
        ...
        let dp = calculate_pressure_drop(flow_rate, length, diameter, reynolds);
    ...
    fn test_elevation_pressure_drop() {
        let dp = calculate_elevation_pressure_drop(2.0, 600.0);
